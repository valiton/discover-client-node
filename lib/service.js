...
Service.prototype._watch = function _watch() {
  ...
  this.watcher.on('change', function (result) {
    ...
    winston.log('debug', 'A registered service changed for service: %s, new service list is: %s', self.servicePath, services, {});
  });

  this.watcher.on('reconnect', function () {
    winston.log('debug', 'Service change watcher reconnected for service: %s', self.servicePath);
  });

  this.watcher.on('error', function (err) {
    winston.log('debug', 'Service change watcher for service: %s threw an error: %s', self.servicePath, err, {});
  });
};
